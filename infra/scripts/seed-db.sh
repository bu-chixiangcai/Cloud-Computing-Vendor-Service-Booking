...
# Idempotent: skips the import if vendor_services_db.users already exists, so
...
ALREADY_SEEDED=$(mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" -N -e \
  "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='vendor_services_db' AND table_name='users'")

if [ "$ALREADY_SEEDED" -gt 0 ]; then
  echo "vendor_services_db.users already exists - database already seeded, skipping import."
  if [ -n "$BUCKET_NAME" ]; then
    ...
    mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" -D "vendor_services_db" -e \
      "UPDATE vendors SET image_url = REPLACE(image_url, '/uploads/', '${S3_PREFIX}') WHERE image_url LIKE '/uploads/%';"
...
if [ -n "$BUCKET_NAME" ]; then
  # Rewrite local image paths to S3 URLs for the sample vendors
  S3_PREFIX="https://${BUCKET_NAME}.s3.${AWS_REGION}.amazonaws.com/uploads/"
  mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" -D "vendor_services_db" -e \
    "UPDATE vendors SET image_url = REPLACE(image_url, '/uploads/', '${S3_PREFIX}') WHERE image_url LIKE '/uploads/%';"
fi
